/**
 * Test ICP con Backtest AVANZATO
...
async function testICPAdvanced() {
    try {
        console.log('\n🚀 BACKTEST AVANZATO ICP - CON LOGICA COMPLETA DEL BOT\n');
        ...
        const analyzer = new AdvancedBacktestAnalyzer('icp', 1000, 100);
        const result = await analyzer.runBacktest(60);

        // Stampa risultati
        console.log('\n' + '='.repeat(80));
        console.log('📊 RISULTATI ICP - BACKTEST AVANZATO (60 GIORNI)');
        console.log('='.repeat(80));
        ...
        sortedTrades.slice(0, 5).forEach((t, idx) => {
            ...
            console.log(`   ${idx + 1}. ${t.type} @ $${t.entryPrice.toFixed(4)} → $${t.exitPrice.toFixed(4)} | +$${t.pnl.toFixed(2)} (${t.pnlPct.toFixed(2)}%) | ${duration}min | ${t.reason}`);
        });
        ...
        sortedTrades.slice(-5).reverse().forEach((t, idx) => {
            const duration = ((t.exitTime - t.openTime) / (1000 * 60)).toFixed(0);
            console.log(`   ${idx + 1}. ${t.type} @ $${t.entryPrice.toFixed(4)} → $${t.exitPrice.toFixed(4)} | -$${Math.abs(t.pnl).toFixed(2)} (${t.pnlPct.toFixed(2)}%) | ${duration}min | ${t.reason}`);
        });
        ...
        // Salva risultati
        const fs = require('fs');
        fs.writeFileSync('./icp_advanced_backtest.json', JSON.stringify({
            symbol: 'ICP',
        ...
        console.log('\n' + '='.repeat(80));
        console.log('✅ Test AVANZATO completato!');
        console.log('📁 Risultati salvati in: icp_advanced_backtest.json');
        console.log('='.repeat(80) + '\n');
