...
// ✅ Endpoint pubblici per AI accesso database (DOPO cryptoRoutes, PRIMA delle route protette)
...
app.get('/api/ai-db/execute', async (req, res) => {
    try {
        ...
        console.error('❌ Error in /ai-db/execute:', err.message);
...
app.post('/api/ai-db/update', async (req, res) => {
    try {
        ...
        console.error('❌ Error in /ai-db/update:', err.message);
        res.status(500).json({ error: err.message });
    }
});
