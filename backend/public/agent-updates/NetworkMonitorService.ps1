...
# Evita lettura/scrittura di config.json nella directory di lavoro sbagliata (es. System32) se manca -ConfigPath assoluto
if ($ConfigPath -and -not [System.IO.Path]::IsPathRooted($ConfigPath)) {
    $ConfigPath = Join-Path $script:scriptDir $ConfigPath
}
    ...
    try {
        ...
        if ($RouterModel -match '^Unifi|^Ubiquiti|^UCK') {
            ...
            try {
                ...
                Write-Log "Risultato Controller WiFi inviato (success=$($errMsg -eq ''), devices=$($devices.Count), error=$errMsg)" "INFO"
            }
            catch { Write-Log "Invio risultato Controller WiFi fallito: $_" "WARN" }
            return
        }
...
function Ensure-TrayFiles {
    ...
    $trayFiles = @(
        ...
        @{ Name = "Riavvia_Agent.bat"; Url = "$baseUrl/api/network-monitoring/download/agent/Riavvia_Agent.bat" }
    )
    ...
    try {
        ...
        if ($now -ge $nextHeartbeatTime) {
            ...
            try {
                ...
                if ($heartbeatResult.success -and $heartbeatResult.force_agent_update) {
                    $lv = $heartbeatResult.latest_agent_version
                    $already = ($null -ne $lv -and "$lv".Trim() -eq "$SCRIPT_VERSION")
                    if ($already) {
                        Write-Log "[INFO] Server segnala aggiornamento forzato ma questa copia e' gia' a $SCRIPT_VERSION — salto restart (il server azzera il flag)." "INFO"
                    }
                    else {
        ...
        if ($now -ge $nextScanTime -or $forceScan) {
            ...
            try {
                ...
                # 2. Invio dati al server (anche con lista vuota: il VPS confronta gli IP ricevuti e marca offline + Telegram)
