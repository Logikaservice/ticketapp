...
WshShell.Run "powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File ""CommAgentService.ps1""", 0, False`;
    ...
    router.get('/debug-state', async (req, res) => {
        try {
            ...
            // One-off cleanup for the bogus IPs polluted by the previous bug
            try {
                await pool.query(`DELETE FROM antivirus_info WHERE device_id IN (SELECT id FROM network_devices WHERE ip_address IN ('192.168.100.2', '192.168.100.20', '192.168.100.3', '192.168.100.4', '192.168.100.5'))`);
            } catch (cleanupErr) {
