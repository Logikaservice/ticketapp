...
module.exports = (pool) => {
  ...
  // ENDPOINT: Leggi log PM2 per verificare aggiornamento formato
  router.get('/logs-update-format', async (req, res) => {
    try {
      const { exec } = require('child_process');
      const util = require('util');
      const execPromise = util.promisify(exec);

      // Cerca nei log PM2 (ultimi 200 righe)
      let logs = '';
      try {
        // Prova prima con ticketapp-backend
        const { stdout } = await execPromise('pm2 logs ticketapp-backend --lines 200 --nostream 2>&1 || pm2 logs backend --lines 200 --nostream 2>&1 || echo "PM2 non disponibile"');
        logs = stdout;
      } catch (pm2Err) {
        // Se PM2 non è disponibile o c'è un errore, prova a leggere da file di log se esiste
        const fs = require('fs');
        const path = require('path');
        const logPath = path.join(__dirname, '..', 'logs', 'error.log');
        if (fs.existsSync(logPath)) {
          logs = fs.readFileSync(logPath, 'utf8');
          // Prendi solo le ultime 200 righe
          const lines = logs.split('\n');
          logs = lines.slice(-200).join('\n');
        } else {
          logs = 'Log non disponibili. PM2 non trovato e file di log non disponibile.';
        }
      }

      // Filtra solo i log relativi all'aggiornamento formato
      const updateFormatLogs = logs
        .split('\n')
        .filter(line => line.includes('[UPDATE-FORMAT]') || 
                        line.includes('AGGIORNAMENTO FORMATO') || 
                        line.includes('update-interventi-format'))
        .slice(-50); // Ultimi 50 log filtrati

      // Cerca anche messaggi di completamento
      const completionMessages = logs
        .split('\n')
        .filter(line => line.includes('AGGIORNAMENTO FORMATO COMPLETATO') || 
                        line.includes('Eventi aggiornati:') ||
                        line.includes('Errori:'))
        .slice(-10);

      res.json({
        success: true,
        updateFormatLogs: updateFormatLogs,
        completionMessages: completionMessages,
        allLogs: logs.split('\n').slice(-100), // Ultimi 100 log totali per contesto
        hasUpdateFormatActivity: updateFormatLogs.length > 0 || completionMessages.length > 0
      });
    } catch (err) {
      console.error('Errore lettura log aggiornamento formato:', err);
      res.status(500).json({
        success: false,
        error: err.message,
