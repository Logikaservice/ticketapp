/**
 * Test ETHEREUM con Backtest AVANZATO
...
async function testETHAdvanced() {
    try {
        console.log('\n🚀 BACKTEST AVANZATO ETHEREUM - CON LOGICA COMPLETA DEL BOT\n');
        console.log('='.repeat(80));
        console.log('✅ Include: Filtri professionali, trailing stop, risk management\n');

        // Scarica dati
        await downloadHistoricalData('ETHUSDT', 'ethereum', 60);

        // Esegui backtest AVANZATO
        const analyzer = new AdvancedBacktestAnalyzer('ethereum', 1000, 100);
        const result = await analyzer.runBacktest(60);

        // Stampa risultati
        console.log('\n' + '='.repeat(80));
        console.log('📊 RISULTATI ETHEREUM - BACKTEST AVANZATO (60 GIORNI)');
        console.log('='.repeat(80));
        ...
        console.log('   Return: +2.68%');
        console.log('   Trade: 54');
        console.log('   Win Rate: 46.3%');
        ...
        const improvement = result.summary.totalReturn - 2.68;
        console.log(`\n${improvement >= 0 ? '📈 MIGLIORAMENTO' : '📉 PEGGIORAMENTO'}: ${improvement >= 0 ? '+' : ''}${improvement.toFixed(2)}%`);

        // Salva risultati
        const fs = require('fs');
        fs.writeFileSync('./eth_advanced_backtest.json', JSON.stringify({
            symbol: 'ETH',
            ...
            comparison: {
                simpleBacktest: { return: 2.68, trades: 54, winRate: 46.3 },
        ...
        console.log('\n' + '='.repeat(80));
        console.log('✅ Test AVANZATO Ethereum completato!');
        console.log('📁 Risultati salvati in: eth_advanced_backtest.json');
        console.log('='.repeat(80) + '\n');
