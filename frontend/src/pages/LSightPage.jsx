...
import { Eye, Activity, Search, X, MonitorPlay, Zap, ShieldCheck, User, Plus, Trash2, Server, Building2, ChevronRight, ArrowLeft, WifiOff, Wifi } from 'lucide-react';

// ─────────────────────────────────────────
// Icone di sistema azienda (colori deterministici)
// ─────────────────────────────────────────
const COMPANY_COLORS = [
  'from-indigo-600/30 to-indigo-800/20 border-indigo-500/30 text-indigo-400',
  'from-cyan-600/30 to-cyan-800/20 border-cyan-500/30 text-cyan-400',
  'from-violet-600/30 to-violet-800/20 border-violet-500/30 text-violet-400',
  'from-emerald-600/30 to-emerald-800/20 border-emerald-500/30 text-emerald-400',
  'from-amber-600/30 to-amber-800/20 border-amber-500/30 text-amber-400',
  'from-rose-600/30 to-rose-800/20 border-rose-500/30 text-rose-400',
  'from-sky-600/30 to-sky-800/20 border-sky-500/30 text-sky-400',
  'from-fuchsia-600/30 to-fuchsia-800/20 border-fuchsia-500/30 text-fuchsia-400',
];
...
const LSightPage = ({ onClose, onNavigateHome, currentUser, getAuthHeader }) => {
  ...
  const companiesMap = useMemo(() => {
    ...
    agents.forEach(ag => {
      ...
      const company = map.get(az);
      company.agents.push(ag);
      company.totalCount++;
      if (ag.status === 'online') company.onlineCount++;
    });
    return map;
  }, [agents]);
  ...
  const selectedCompanyAgents = useMemo(() => {
    ...
    const company = companiesMap.get(selectedCompany);
    if (!company) return [];
    return company.agents.filter(ag =>
  ...
  const handleAssignAgent = async () => {
    if (!selectedUserForAssign || !selectedAgentForAssign) return alert('Seleziona utente e agent!');
    ...
    if (res.ok) { fetchAdminData(); setSelectedUserForAssign(''); setSelectedAgentForAssign(''); }
    else alert('Errore in assegnazione');
  };
  ...
  const PcCard = ({ ag }) => {
    ...
    return (
      <div className={`group relative bg-[#111827] rounded-2xl border transition-all overflow-hidden flex flex-col shadow-lg ${isOnline ? 'border-indigo-500/30 hover:border-indigo-400/60' : 'border-slate-800 hover:border-slate-700'}`}>
        {isOnline && <div className="absolute -top-4 -right-4 w-24 h-24 bg-indigo-500/10 blur-2xl rounded-full pointer-events-none" />}
  ...
  const CompanyCard = ({ company }) => {
    ...
    const [gradient, border, textColor] = color.split(' ');
    const isActive = selectedCompany === company.name;
    return (
      <button
        onClick={() => { setSelectedCompany(company.name); setPcSearch(''); }}
        className={`group relative text-left w-full rounded-2xl border bg-gradient-to-br p-5 transition-all shadow-lg hover:scale-[1.02] hover:shadow-indigo-500/10 active:scale-[0.99] overflow-hidden ${gradient} ${border} ${isActive ? 'ring-2 ring-indigo-400 ring-offset-2 ring-offset-[#0A0E17]' : ''}`}
      >
        <div className="absolute -bottom-4 -right-4 w-20 h-20 bg-white/5 rounded-full" />
        <div className="flex items-start gap-3">
          <div className={`w-10 h-10 rounded-xl bg-black/30 flex items-center justify-center text-sm font-black ${textColor} flex-shrink-0 border border-white/10`}>
      ...
      <header className="flex-none bg-[#0D131F] border-b border-indigo-900/30 px-6 py-4 flex items-center justify-between shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 relative z-10">
          {isTecnico && (
            ...
            >
              <Zap size={16} /> Assegnazioni
            </button>
          )}
          <button onClick={onNavigateHome || onClose} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/10 transition-all text-sm font-medium">
            <X size={18} /> Chiudi
          </button>
        </div>
      </header>
            ...
            <>
              ...
              {companies.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 px-4 text-center border border-dashed border-slate-800 rounded-2xl bg-[#0D131F]/50">
                  ...
                  <p className="text-slate-400 text-sm max-w-sm">Non ci sono agent con un'azienda associata, oppure nessun PC è ancora stato assegnato al tuo profilo.</p>
                </div>
              ) : (
                /* 4 colonne */
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {companies.map(co => <CompanyCard key={co.name} company={co} />)}
                </div>
              )}
            </>

          ) : (
            /* ── STEP 2: PC list for company ── */
            <>
              {/* Barra navigazione + ricerca PC */}
              <div className="flex items-center gap-3 flex-wrap">
                ...
                >
                  <ArrowLeft size={16} /> Tutte le aziende
                </button>
              ...
              {selectedCompanyAgents.length === 0 ? (
                ...
                /* 4 colonne PC */
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {selectedCompanyAgents.map(ag => <PcCard key={ag.agent_id} ag={ag} />)}
                </div>
              )}
            </>
          )}
