# Script completo per deploy agent v1.1.4

Write-Host "=== DEPLOY AGENT v1.1.4 ===" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "2. Creazione directory sulla VPS..." -ForegroundColor Yellow
ssh "${vpsUser}@${vpsHost}" "mkdir -p /var/www/ticketapp/agent-updates"
Write-Host "   OK: Directory creata/verificata" -ForegroundColor Green
...
$sshCommand = @"
cd /var/www/ticketapp/agent-updates && echo '{\"version\": \"1.1.4\", \"download_url\": \"/agent-updates/agent-update-1.1.4.zip\"}' > version.json && cat version.json
"@
...
Write-Host ""
Write-Host "Gli agent si aggiorneranno automaticamente alla versione 1.1.4" -ForegroundColor Cyan
