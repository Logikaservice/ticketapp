...
router.post('/cleanup-positions', async (req, res) => {
    try {
        ...
        const targetMax = max_positions && !isNaN(parseInt(max_positions)) ? parseInt(max_positions) : null;
        const result = await performCleanupPositions(targetMax);
        res.json(result);
    } catch (err) {
...
// ✅ FIX: Endpoint health-status (originale)
router.get('/health-status', healthCheckHandler);

// ✅ FIX: Alias system-health per compatibilità frontend
router.get('/system-health', healthCheckHandler);
...
const runBotCycleForSymbol = async (symbol, botSettings) => {
    try {
        ...
        if (signal.atrBlocked) {
        ...
        } else if (signal.direction === 'LONG' && signal.strength >= MIN_SIGNAL_STRENGTH && (signal.confirmations || 0) < MIN_CONFIRMATIONS_LONG) {
            // ✅ Verifica conferme insufficienti
            const missingConfirmations = MIN_CONFIRMATIONS_LONG - (signal.confirmations || 0);
            console.log(`\n🛑 [BLOCCATO] ${symbol.toUpperCase()}: Conferme insufficienti per LONG`);
            console.log(`   🔴 Conferme attuali: ${signal.confirmations || 0}/${MIN_CONFIRMATIONS_LONG} (mancano ${missingConfirmations})`);
            console.log(`   💡 Il bot aspetta più conferme tecniche prima di aprire\n`);
        } else if (!signal.atrBlocked && signal.direction === 'LONG' && signal.strength >= MIN_SIGNAL_STRENGTH && (signal.confirmations || 0) >= MIN_CONFIRMATIONS_LONG) {
            ...
            // ✅ Verifica anche che le conferme siano sufficienti
        ...
        } else if (signal.direction === 'SHORT' && signal.strength >= MIN_SIGNAL_STRENGTH && (signal.confirmations || 0) < MIN_CONFIRMATIONS_SHORT) {
            // ✅ Verifica conferme insufficienti per SHORT
            const missingConfirmations = MIN_CONFIRMATIONS_SHORT - (signal.confirmations || 0);
            console.log(`\n🛑 [BLOCCATO] ${symbol.toUpperCase()}: Conferme insufficienti per SHORT`);
            console.log(`   🔴 Conferme attuali: ${signal.confirmations || 0}/${MIN_CONFIRMATIONS_SHORT} (mancano ${missingConfirmations})`);
            console.log(`   💡 Il bot aspetta più conferme tecniche prima di aprire\n`);
        }
        else if (!signal.atrBlocked && !portfolioDrawdownBlock && !marketRegimeBlock && signal.direction === 'SHORT' && signal.strength >= MIN_SIGNAL_STRENGTH && (signal.confirmations || 0) >= MIN_CONFIRMATIONS_SHORT) {
...
// POST /api/crypto/positions/update-pnl - Update P&L for all positions (called from frontend)
router.post('/positions/update-pnl', async (req, res) => {
    try {
        // ✅ FIX: updatePositionsPnL aggiorna TUTTE le posizioni, recuperando i prezzi da Binance per ciascuna
        const updatedCount = await updatePositionsPnL();
        res.json({
            success: true,
        ...
        console.error('❌ [UPDATE-PNL-POST] Error updating P&L:', err.message);
        res.status(500).json({ error: err.message });
    }
});
    ...
    try {
        ...
        if (globalBot && globalBot.parameters) {
            ...
            console.log('📥 [BOT-PARAMS-GET] Parametri RAW dal database:', {
                ...
                trade_size_eur: globalParams.trade_size_eur,
                max_positions: globalParams.max_positions,
                hasTradeSizeUsdt: 'trade_size_usdt' in globalParams,
...
router.put('/bot/parameters', async (req, res) => {
    ...
    try {
        ...
        let finalParams = { ...validParams }; // ✅ FIX: Inizia da validParams (contiene i valori validati)
        if (existing && existing.parameters) {
            ...
            for (const key in existingGlobalParams) {
                if (existingGlobalParams[key] !== undefined && existingGlobalParams[key] !== null) {
            ...
            console.log('🔄 [BOT-PARAMS] Merge con parametri esistenti:', {
                ...
                test_trade_size_after_merge: finalParams.test_trade_size,
...
router.get('/bot-analysis', async (req, res) => {
    ...
    try {
        ...
        // ✅ FIX: Se trade_size_usdt è configurato, usa quello SEMPRE
        const configuredTradeSize = params.trade_size_usdt || params.trade_size_eur || null;
        const maxAvailableForNewPosition = configuredTradeSize || 100; // Default $100

        const canOpenCheck = { allowed: true, reason: 'OK' }; // RiskManager rimosso
        ...
        // ✅ FIX: Aggiungi tradingViewSymbol per TradingView (converte EUR -> USDT)
        // TradingView non supporta coppie EUR, quindi convertiamo in USDT
        ...
        // Ottieni il simbolo per TradingView
        const symbolForTradingView = SYMBOL_TO_PAIR[symbol] || symbol.toUpperCase();
        const tradingViewSymbol = tradingViewSymbolMap[symbolForTradingView] || 
                                  (symbolForTradingView.endsWith('EUR') ? 
                                   symbolForTradingView.replace('EUR', 'USDT') : 
                                   symbolForTradingView);
        
        responseData.tradingViewSymbol = tradingViewSymbol;
        responseData.originalSymbol = symbol;
        responseData.symbolForChart = tradingViewSymbol; // Alias per compatibilità
