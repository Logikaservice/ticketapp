...
async function openPosition(symbol, type, volume, entryPrice, strategy, stopLoss, takeProfit, signalDetails) {
    try {
        ...
        // ✅ FIX: Calcola tradeSize come volume * entryPrice invece di usare variabile non definita
        const tradeSize = volume * entryPrice;
...
async function runBotCycleForSymbol(symbol, botSettings) {
    try {
        ...
        if (klines.length < 50) {
            console.warn(`⚠️ [BOT] Insufficient klines for ${symbol} (${klines.length}/50), attempting regeneration...`);

            try {
                // Lazy load to avoid circular dependency issues
                const DataIntegrityService = require('./DataIntegrityService');
                const regenerationResult = await DataIntegrityService.verifyAndRegenerate(symbol);

                if (regenerationResult.regenerated || regenerationResult.valid) {
                    // Retry fetch after regeneration
                    const klinesRetry = await dbAll(
                        "SELECT * FROM klines WHERE symbol = $1 AND interval = '15m' ORDER BY open_time DESC LIMIT 100",
                        [symbol]
                    );

                    if (klinesRetry.length >= 50) {
                        console.log(`✅ [BOT] Klines regenerated successfully for ${symbol}`);
                        // Update the klines variable to continue with analysis
                        klines.length = 0; // Clear array
                        klines.push(...klinesRetry); // Add new data
                    } else {
                        console.error(`❌ [BOT] Regeneration failed or insufficient data even after retry for ${symbol}`);
                        return;
                    }
                } else {
                    console.warn(`⚠️ [BOT] Regeneration skipped or failed for ${symbol}`);
                    return;
                }
            } catch (err) {
                console.error(`❌ [BOT] Error during data regeneration for ${symbol}:`, err.message);
                return;
            }
        }
...
// ✅ DISABILITATO: Il bot principale ora è in cryptoRoutes.js
// Il bot vecchio viene disabilitato per evitare conflitti e bug (tradeSize is not defined)
// Start the bot loop
