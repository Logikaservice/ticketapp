...
const BotAnalysisPageNew = () => {
    ...
    const preservedSwotAnalysisRef = React.useRef(null);
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            // ✅ FIX: Preserva signal.reasons, blockers e swot_analysis per evitare che scompaiano durante l'aggiornamento
            // Se i nuovi dati hanno reasons/blockers/swot_analysis validi, usali. Altrimenti, mantieni quelli precedenti
            const newReasons = jsonData.signal?.reasons;
            const newBlockers = jsonData.blockers;
            const newSwotAnalysis = jsonData.swot_analysis;
            ...
            if (!preservedSwotAnalysisRef.current && data?.swot_analysis) {
                preservedSwotAnalysisRef.current = data.swot_analysis;
            }
            ...
            if (oldDirection && newDirection && oldDirection !== newDirection) {
                ...
                if (!hasNewBlockers) {
                    ...
                    // Non resettare swot_analysis perché può essere utile anche dopo il cambio di direzione
                }
            }
            ...
            // ✅ Aggiorna swot_analysis preservata SOLO se i nuovi dati hanno swot_analysis valida
            if (newSwotAnalysis && typeof newSwotAnalysis === 'object') {
                preservedSwotAnalysisRef.current = newSwotAnalysis;
            }
            ...
            // ✅ Determina quale swot_analysis usare: preferisci i nuovi se validi, altrimenti usa i preservati
            const finalSwotAnalysis = (newSwotAnalysis && typeof newSwotAnalysis === 'object')
                ? newSwotAnalysis
                : (preservedSwotAnalysisRef.current)
                    ? preservedSwotAnalysisRef.current
                    : (data?.swot_analysis) // Fallback ai dati esistenti
                        ? data.swot_analysis
                        : null;
            ...
            const freshData = {
                ...
                // ✅ Usa swot_analysis finale calcolata sopra
                swot_analysis: finalSwotAnalysis
            };
    ...
    const { signal, requirements, risk, positions, currentPrice, rsi, mtf, swot_analysis } = data;
        ...
        <div className="bot-analysis-page" data-new-version="true">
            <div className="page-container">
                ...
                <div className="page-content">
                    ...
                    {/* 🚀 STRATEGIA INNOVATIVA v2.0 - Williams %R + TSI + RSI */}
                    {signal && signal.indicators && (signal.indicators.williamsR !== undefined || signal.indicators.tsi !== undefined) && (
                        <div className="innovative-strategy-section" key={`innovative-${updateKey}`}>
                            <h3>🚀 STRATEGIA INNOVATIVA v2.0 - Analisi in Tempo Reale</h3>
                            <div className="strategy-cards">
                                {/* Williams %R */}
                                {signal.indicators.williamsR !== undefined && (
                                    <div className="strategy-card williams-r">
                                        <div className="strategy-header">
                                            <span className="strategy-icon">📊</span>
                                            <span className="strategy-name">Williams %R</span>
                                        </div>
                                        <div className="strategy-value">
                                            {signal.indicators.williamsR?.toFixed(2)}
                                        </div>
                                        <div className="strategy-interpretation">
                                            {signal.indicators.williamsR > -20 ? '📈 Overbought (possibile calo)' :
                                             signal.indicators.williamsR < -80 ? '📉 Oversold (possibile rialzo)' :
                                             '➡️ Neutrale'}
                                        </div>
                                        <div className="strategy-description">
                                            Indicatore di momentum che identifica condizioni di oversold/overbought più velocemente dell'RSI
                                        </div>
                                    </div>
                                )}

                                {/* TSI */}
                                {signal.indicators.tsi !== undefined && (
                                    <div className="strategy-card tsi">
                                        <div className="strategy-header">
                                            <span className="strategy-icon">📈</span>
                                            <span className="strategy-name">TSI (True Strength Index)</span>
                                        </div>
                                        <div className="strategy-value">
                                            {signal.indicators.tsi?.toFixed(2)}
                                        </div>
                                        <div className="strategy-interpretation">
                                            {signal.indicators.tsi > 25 ? '📈 Forte momentum rialzista' :
                                             signal.indicators.tsi < -25 ? '📉 Forte momentum ribassista' :
                                             '➡️ Momentum neutrale'}
                                        </div>
                                        <div className="strategy-description">
                                            Misura l'accelerazione del momentum con doppio smoothing per segnali più affidabili
                                        </div>
                                    </div>
                                )}

                                {/* Triple Momentum Signal */}
                                {signal.indicators.williamsR !== undefined && signal.indicators.tsi !== undefined && rsi && (
                                    <div className="strategy-card triple-momentum">
                                        <div className="strategy-header">
                                            <span className="strategy-icon">🎯</span>
                                            <span className="strategy-name">Triple Momentum Signal</span>
                                        </div>
                                        <div className="strategy-analysis">
                                            <div className="momentum-indicators">
                                                <div className="momentum-item">
                                                    <span>RSI:</span>
                                                    <span className={`momentum-value ${rsi < 30 ? 'bullish' : rsi > 70 ? 'bearish' : 'neutral'}`}>
                                                        {rsi?.toFixed(2)}
                                                    </span>
                                                </div>
                                                <div className="momentum-item">
                                                    <span>Williams %R:</span>
                                                    <span className={`momentum-value ${signal.indicators.williamsR < -80 ? 'bullish' : signal.indicators.williamsR > -20 ? 'bearish' : 'neutral'}`}>
                                                        {signal.indicators.williamsR?.toFixed(2)}
                                                    </span>
                                                </div>
                                                <div className="momentum-item">
                                                    <span>TSI:</span>
                                                    <span className={`momentum-value ${signal.indicators.tsi > 25 ? 'bullish' : signal.indicators.tsi < -25 ? 'bearish' : 'neutral'}`}>
                                                        {signal.indicators.tsi?.toFixed(2)}
                                                    </span>
                                                </div>
                                            </div>
                                            <div className="momentum-summary">
                                                <strong>Analisi Combinata:</strong>
                                                {(() => {
                                                    const bullishCount = (rsi < 30 ? 1 : 0) + (signal.indicators.williamsR < -80 ? 1 : 0) + (signal.indicators.tsi > 25 ? 1 : 0);
                                                    const bearishCount = (rsi > 70 ? 1 : 0) + (signal.indicators.williamsR > -20 ? 1 : 0) + (signal.indicators.tsi < -25 ? 1 : 0);

                                                    if (bullishCount >= 2) {
                                                        return <span className="momentum-signal bullish">🔥 Segnale rialzista forte ({bullishCount}/3 indicatori)</span>;
                                                    } else if (bearishCount >= 2) {
                                                        return <span className="momentum-signal bearish">❄️ Segnale ribassista forte ({bearishCount}/3 indicatori)</span>;
                                                    } else if (bullishCount === 1 && bearishCount === 0) {
                                                        return <span className="momentum-signal bullish">📈 Segnale rialzista debole (1/3 indicatori)</span>;
                                                    } else if (bearishCount === 1 && bullishCount === 0) {
                                                        return <span className="momentum-signal bearish">📉 Segnale ribassista debole (1/3 indicatori)</span>;
                                                    } else {
                                                        return <span className="momentum-signal neutral">⚖️ Segnali contrastanti - attendere conferma</span>;
                                                    }
                                                })()}
                                            </div>
                                        </div>
                    ...
                    {/* SWOT Analysis */}
                    {swot_analysis && (
                        <div className="swot-section" key={`swot-${updateKey}`}>
                            <h3>🔍 SWOT Analysis</h3>
                            <div className="swot-grid">
                                <div className="swot-card strengths">
                                    <h4>💪 Strengths</h4>
                                    <ul>
                                        {swot_analysis.strengths?.map((strength, idx) => (
                                ...
                                <div className="swot-card weaknesses">
                                    <h4>⚠️ Weaknesses</h4>
                                    <ul>
                                        {swot_analysis.weaknesses?.map((weakness, idx) => (
                                ...
                                <div className="swot-card opportunities">
                                    <h4>🚀 Opportunities</h4>
                                    <ul>
                                        {swot_analysis.opportunities?.map((opportunity, idx) => (
                                ...
                                <div className="swot-card threats">
                                    <h4>⚡ Threats</h4>
                                    <ul>
                                        {swot_analysis.threats?.map((threat, idx) => (
